...
struct MonitoredApp: Codable, Identifiable, Equatable {
  ...
  init(
    bundleIdentifier: String, name: String, iconPath: String? = nil,
    activationDelay: TimeInterval = 5.0, customMessage: String? = nil, isLocked: Bool? = nil,
...
struct ScheduledTime: Codable, Identifiable, Equatable {
  ...
  init(
    id: UUID = UUID(), date: Date, name: String = "", isRecurring: Bool = true,
    isLocked: Bool? = nil, customDuration: Int? = nil, repeatDays: Set<Int> = [],
    isEnabled: Bool = true
  ) {
    self.id = id
    self.date = date
...
struct NoGoTime: Codable, Identifiable, Equatable {
  ...
  init(
    id: UUID = UUID(), startTime: Date, endTime: Date, name: String = "", repeatDays: Set<Int> = [],
    isEnabled: Bool = true
  ) {
    self.id = id
...
class Settings: ObservableObject {
  ...
  private init() {
    ...
    self.selectedAmbientSound =
      UserDefaults.standard.object(forKey: "selectedAmbientSound") as? String ?? "random"
    self.startSoundEnabled =
      UserDefaults.standard.object(forKey: "startSoundEnabled") as? Bool ?? true
    self.startSoundVolume =
      UserDefaults.standard.object(forKey: "startSoundVolume") as? Double ?? 0.5
    ...
    self.sessionDisplayText =
      UserDefaults.standard.object(forKey: "sessionDisplayText") as? String ?? "Just Breathe"
    self.lockSessionEnabled =
      UserDefaults.standard.object(forKey: "lockSessionEnabled") as? Bool ?? false
    self.completedSessions = UserDefaults.standard.object(forKey: "completedSessions") as? Int ?? 0
    self.completedSessionTime =
      UserDefaults.standard.object(forKey: "completedSessionTime") as? Int ?? 0
    ...
    self.appLaunchEnabled =
      UserDefaults.standard.object(forKey: "appLaunchEnabled") as? Bool ?? false
    ...
    if let modifiers = UserDefaults.standard.object(forKey: "activateHotkeyModifiers") as? UInt32 {
      ...
      self.activateHotkeyModifiers =
        UserDefaults.standard.object(forKey: "hotkeyModifiers") as? UInt32 ?? defaultModifiers
    }

    if let keyCode = UserDefaults.standard.object(forKey: "activateHotkeyKeyCode") as? UInt32 {
      self.activateHotkeyKeyCode = keyCode
    } else {
      self.activateHotkeyKeyCode =
        UserDefaults.standard.object(forKey: "hotkeyKeyCode") as? UInt32 ?? defaultKeyCode
    }

    let defaultExitModifiers: UInt32 = 0  // No modifiers
    let defaultExitKeyCode: UInt32 = 14
    self.exitHotkeyModifiers =
      UserDefaults.standard.object(forKey: "exitHotkeyModifiers") as? UInt32 ?? defaultExitModifiers
    self.exitHotkeyKeyCode =
      UserDefaults.standard.object(forKey: "exitHotkeyKeyCode") as? UInt32 ?? defaultExitKeyCode
    ...
    self.snoozeHotkeyModifiers =
      UserDefaults.standard.object(forKey: "snoozeHotkeyModifiers") as? UInt32
      ?? defaultSnoozeModifiers
    self.snoozeHotkeyKeyCode =
  ...
  func isInNoGoTime() -> Bool {
    ...
    for noGoTime in noGoTimes {
      ...
      guard let todayStart = calendar.date(from: todayStartComponents),
        var todayEnd = calendar.date(from: todayEndComponents)
      else {
        continue
      }
